...
class DatasetCutConcatNewMergeTwo():
    ...
    def __getitem__(self, index):
        ...
        feature[:, :data_len, :data_len] = np.concatenate([temp, np.transpose(temp, [1, 0, 2])], 2)
        feature[:, :data_len, :data_len] = feature[:, :data_len, :data_len].reshape((-1, data_len, data_len))
